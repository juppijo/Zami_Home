...
const cacheBtnLabel    = $('cacheBtnLabel');
...
function buildThumbCard(entry, index) {
  ...
  const loadImage = async () => {
    ...
    // Thumbnail-Cache liefert sofort gecachtes Bild oder erstellt + speichert es
    let src;
    try {
      src = await TC.getThumbnail(entry);
    } catch {
      // Fallback: Originaldatei direkt
      src = entry.isUrl ? entry.url
          : (entry._objUrl || (entry._objUrl = URL.createObjectURL(entry.file)));
    }
    ...
    img.onload  = () => {
      ...
      // Cache-Label asynchron aktualisieren
      updateCacheBtnLabel();
    };
    img.onerror = () => {
      // Cache-Eintrag ungültig → löschen und Originalquelle versuchen
      TC.remove(entry);
      const fallback = entry.isUrl ? entry.url
          : (entry._objUrl || (entry._objUrl = URL.createObjectURL(entry.file)));
      if (img.src !== fallback) {
        img.src = fallback;
      } else {
...
═════════════════════════════════════════════════════ */
let _cacheMenuOpen = false;

async function updateCacheBtnLabel() {
  try {
    const s = await TC.getStats();
    if (s.count === 0) {
      cacheBtnLabel.textContent = 'Cache';
    } else {
      cacheBtnLabel.textContent = `${s.count} · ${formatBytes(s.bytes)}`;
    }
  } catch { cacheBtnLabel.textContent = 'Cache'; }
}
...
cacheBtn.addEventListener('click', async () => {
  ...
  const msg = s.count === 0
    ? 'Cache ist leer.'
    : `Cache: ${s.count} Thumbnails · ${formatBytes(s.bytes)}\nJetzt leeren?`;

  if (s.count === 0) { showToast(msg); return; }

  // Einfaches Confirm-Dialog
  if (confirm(`Cache enthält ${s.count} Thumbnails (${formatBytes(s.bytes)}).\nCache leeren?`)) {
    await TC.clear();
    updateCacheBtnLabel();
